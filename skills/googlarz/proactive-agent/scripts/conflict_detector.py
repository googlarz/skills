    ...
    for i in range(len(timed)):
        for j in range(i + 1, len(timed)):
            ...
            conflicts.append({
                ...
                "events": [ev1.get("title", ""), ev2.get("title", "")],
                "overlap_minutes": overlap_min,
                "message": (
                    f"⚠️ Conflict: *{ev1.get('title','')}* and *{ev2.get('title','')}* "
...
def detect_overloaded_days(events: list, threshold: int = 4) -> list:
    ...
    for date_str, evs in day_events.items():
        if len(evs) >= threshold:
            titles = ", ".join(e.get("title", "") for e in evs[:3])
            more = len(evs) - 3
            overloaded.append({
                "type": "overloaded_day",
...
def detect_back_to_back(events: list, gap_minutes: int = 10) -> list:
    ...
    for i in range(1, len(timed)):
        ...
        if gap <= gap_minutes:
            ...
            if len(run) >= 3:
                titles = " → ".join(e[2].get("title", "") for e in run)
                conflicts.append({
                    "type": "back_to_back",
